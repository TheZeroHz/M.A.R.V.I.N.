#include <Motion_Detector_inferencing.h>
...
#define MAX_ACCEPTED_RANGE  2.0f        // starting 03/2022, models are generated setting range to +-2,
                                        // but this example use Arduino library which set range to +-4g.
...
static bool ei_connect_fusion_list(const char *input_list)
{
    ...
    while(buff != NULL) {
        int8_t res_ix = ei_find_axis(buff);

        if(res_ix == -1) {
            ei_printf("ERR: Axis name \"%s\" not found\n", buff);
            return false;
        }

        fusion_sensors[fusion_ix++] = res_ix;
        sensors[res_ix].status = 0;
        is_fusion = true;
    ...
    if (is_fusion == false) {
        ei_printf("ERR: Empty sensor list\r\n");
    }
 ...
 *
 * @retval true if successful init
 */
bool init_IMU(void)
{
...
/**
 * @brief Poll IMU sensor, retrieve accel data
 *
 * @retval 0 if successful poll
 */
uint8_t poll_IMU(void)
{
        xyzFloat acc = myMPU9250.getGValues();
        data[0] = acc.x * CONVERT_G_TO_MS2;
        data[1] = acc.y * CONVERT_G_TO_MS2;
        data[2] = acc.z * CONVERT_G_TO_MS2;
        return 0;
}

/**
 * @brief Init ADC sensor
 *
 * @retval true if successful init
 */
bool init_ADC(void)
{
    /* ADC init is default done automatically, no need further action */
    return true;
}

/**
 * @brief Poll ADC sensor
 *
 * @retval 0 if successful poll
 */
uint8_t poll_ADC(void)
